jQuery(function($){
  ...
  var psForm = {
    ...
    onSubmit: function(e){
      if(!psForm.hasToken() && psForm.payWithPaymentSpring() && public_key !== "false"){
        e.preventDefault();
    ...
    errorMessage: function(errors){
      return $.map(response.errors, function(error){
    ...
    onFailure: function(response){
      ...
      $('#paymentspring-card-number').closest( 'p' ).before( 
        '<ul class="woocommerce_error woocommerce-error wc-paymentspring-error"><li>' + this.errorMessage(response["errors"]["message"]) + '</li></ul>' 
      );
      psForm.form.unblock();
    }
  }
