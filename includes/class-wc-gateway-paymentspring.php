<?php
  ...
  class WC_Gateway_PaymentSpring extends WC_Payment_Gateway_CC{
    ...
    public function __construct(){
      $this->method_title = __("PaymentSpring", "wc_paymentspring");
      $this->id = "paymentspring";
    ...
    public function init_form_fields(){
      $this->form_fields = include('settings-paymentspring.php');
    }
    ...
    public function payment_scripts(){
      ...
      wp_enqueue_script('paymentspring_js_api', plugins_url('assets/js/paymentspring.js', PAYMENT_SPRING_WC_FILE), true);
      wp_enqueue_script('paymentspring_woocommerce', plugins_url('assets/js/paymentspring-form.js', PAYMENT_SPRING_WC_FILE), true);
      ...
      wp_localize_script('paymentspring_woocommerce', 'public_key', apply_filters('public_key', $apiKey));
    }
    ...
    public function process_payment( $order_id, $retry = true, $force_customer = false ) {
      ...
      if(!$this->keysAreSet){
        throw new Exception(
          __("Please set your API Keys in WooCommerce > Settings > Checkout > PaymentSpring", 'paymentspring-woocommerce-gateway')
        );
      }
      ...
      if($order->get_total() > 0){
        if(get_current_user_id()){
          ...
          $description = __("Payment made via WooCommerce");
          ...
          $description = __("Payment made via WooCommerce");
          $response = $this->chargeToken($token, $order);
        }
      ...
      do_action( 'wc_gateway_paymentspring_process_payment', $response, $order );
    ...
    public function get_customer_id($order = null, $token = null){
      $customerId = get_user_meta(get_current_user_id(), "_paymentspring_customer_id_".$this->apiMode, true);
      if($token && !$customerId){
        ...
        update_user_meta(get_current_user_id(), '_paymentspring_customer_id', $customerResponse->id );
        $customerId = $customerResponse->id;
      }else if(!$customerId && !$token){
        throw new Exception(__("PaymentSprint requires token or customer to process", "paymentspring-woocommerce-gateway"));
      }
      return $customerId;
    }
    ...
    public function chargeCustomer($customerId, $order, $description){
      ...
      return \PaymentSpring\Charge::chargeCustomer($customerId, $amount_in_cents, array("description" => $description));
    }

    public function chargeToken($token, $order, $description){
      $amount = $order->get_total();
      $amount_in_cents = $amount * 100;
      return \PaymentSpring\Charge::chargeToken($token, $amount_in_cents, array("description" => $description));
    }
    ...
    // Sets payment susccess messages based on the response
    // This also handles a failes payment and will place an order as "on hold"
    protected function process_payment_success($response, $order){
      update_post_meta( $order->id, '_paymentspring_charge_id', $response->id );
      update_post_meta( $order->id, '_paymentspring_charge_captured', $response->status == "SETTLED" ? 'yes' : 'no' );
      
      if($response->status == "SETTLED"){
        $order->payment_complete( $response->id ); 
        $message = sprintf( __( 'PaymentSpring charge complete (Charge ID: %s)', 'paymentspring-woocommerce-gateway' ), $response->id );
        $order->add_order_note( $message );
      }else{
        $oder->update_status('on-hold');
        $message = sprintf( __( 'PaymentSpring charge complete (Charge ID: %s) with status %s', 'paymentspring-woocommerce-gateway' ), $response->id, $response->status );
        $order->add_order_note($message);
      }
      return $response;
    }
